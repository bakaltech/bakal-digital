...
const CapabilityLeadCard: React.FC<{ category: CategoryCardProps['category'] }> = ({ category }) => (
  <motion.div
    ...
    className="relative min-h-[420px] sm:min-h-[480px] rounded-[2rem] sm:rounded-[2.5rem] overflow-hidden border border-brand-100/50 bg-ink shadow-[0_28px_70px_rgba(17,19,21,0.14)]"
  >
    <CategoryVisual visual={category.visual} icon={category.icon} />
    ...
    <div className="relative z-10 h-full p-6 sm:p-8 md:p-10 flex flex-col">
      ...
      <h3 className="text-4xl sm:text-5xl font-semibold text-white tracking-tight leading-[0.95] max-w-[8ch] mb-5">{category.title}</h3>
      <p className="text-base sm:text-lg text-white/70 leading-relaxed max-w-[28ch]">{category.description}</p>
